...
// Create page object
$setting_chairman_signature_blobview = new csetting_chairman_signature_blobview();
$Page =& $setting_chairman_signature_blobview;

// Page init
$setting_chairman_signature_blobview->Page_Init();

// Page main
$setting_chairman_signature_blobview->Page_Main();
?>
<?php
$setting_chairman_signature_blobview->Page_Terminate();
?>
<?php
...
// Page class
//
class csetting_chairman_signature_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page object name
	var $PageObjName = 'setting_chairman_signature_blobview';
	...
	//
	function csetting_chairman_signature_blobview() {
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('setting', 'x_chairman_signature');

		// Show thumbnail
		$bShowThumbnail = (@$_GET["showthumbnail"] == "1");
		if (@$_GET["thumbnailwidth"] == "" && @$_GET["thumbnailheight"] == "") {
			$iThumbnailWidth = 120; // Set default width
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				...
				if (trim(strval($this->Recordset->fields('chairman_signature'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $this->Recordset->fields('chairman_signature'));
				}
				$objBinary->Value = $this->Recordset->fields('chairman_signature');
