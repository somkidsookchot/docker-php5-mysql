...
// Create page object
$expenseslist_add = new cexpenseslist_add();
$Page =& $expenseslist_add;

// Page init
$expenseslist_add->Page_Init();

// Page main
$expenseslist_add->Page_Main();
?>
...
// Create page object
var expenseslist_add = new ew_Page("expenseslist_add");

// page properties
expenseslist_add.PageID = "add"; // page ID
expenseslist_add.FormID = "fexpenseslistadd"; // form ID
var EW_PAGE_ID = expenseslist_add.PageID; // for backward compatibility

// extend page with ValidateForm function
expenseslist_add.ValidateForm = function(fobj) {
...
expenseslist_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
expenseslist_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
expenseslist_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
expenseslist_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<div class="phpmaker ewTitle"><img src="images/ico_expenses_add.png" width="40" height="40" align="absmiddle" />&nbsp;<?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $expenseslist->TableCaption() ?></div>
<div class="clear"></div>
<?php $expenseslist_add->ShowPageHeader(); ?>
<?php
$expenseslist_add->ShowMessage();
?>
<form name="fexpenseslistadd" id="fexpenseslistadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return expenseslist_add.ValidateForm(this);">
<p>
...
<p>
<a href="<?php echo $expenseslist->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a>&nbsp;<input type="submit" name="btnAction" id="btnAction" value="<?php echo ew_BtnCaption($Language->Phrase("AddBtn")) ?>">
</form>
<?php
$expenseslist_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$expenseslist_add->Page_Terminate();
?>
<?php
...
class cexpenseslist_add {
	...
	// Page object name
	var $PageObjName = 'expenseslist_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$expenseslist->CurrentAction = $_POST["a_add"]; // Get form action
			$this->CopyRecord = $this->LoadOldRecord(); // Load old recordset
			$this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$expenseslist->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["exp_id"] != "") {
				...
				$expenseslist->setKey("exp_id", $expenseslist->exp_id->CurrentValue); // Set up key
			} else {
				$expenseslist->setKey("exp_id", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
				$expenseslist->CurrentAction = "C"; // Copy record
			} else {
				$expenseslist->CurrentAction = "I"; // Display blank record
		...
		switch ($expenseslist->CurrentAction) {
			...
			case "A": // ' Add new record
				$expenseslist->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $expenseslist->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "expenseslistview.php")
						$sReturnUrl = $expenseslist->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$expenseslist->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		...
		$expenseslist->exp_detail->CurrentValue = NULL;
		$expenseslist->exp_detail->OldValue = $expenseslist->exp_detail->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// exp_detail
		$expenseslist->exp_detail->SetDbValueDef($rsnew, $expenseslist->exp_detail->CurrentValue, "", FALSE);
