// Popup panel for PHP Report Maker 4
// (C) 2006-2010 e.World Technology Ltd.

var EWRPT_POPUP_MINWIDTH = 120;
var EWRPT_POPUP_DEFAULTHEIGHT = 150;
...
function ewrpt_ResizePopup(ele, mdx, mdy) {
	...
	xResizeTo(dd,	Math.max(xWidth(dd) + mdx, EWRPT_POPUP_MINWIDTH), xHeight(dd) + mdy);
...
function ewrpt_SetPopupContent(name, data, useRange, rangeFrom, rangeTo) {
	...
	html += "<div style=\"overflow: auto; height: " + EWRPT_POPUP_DEFAULTHEIGHT + "px;\" id=\"" + name + "_Data\">";
	if (useRange) {
		var selected;
	...
	html += "<input type=\"button\" name=\"OK\" id=\"" + name + "_OK\" value=\"" + ewLanguage.Phrase("PopupOK") + "\" onclick=\"if (!ewrpt_SelectedEntry(this.form,'" + name + "')) alert('" + ewLanguage.Phrase("PopupNoValue") + "'); else {this.form.submit();ewrpt_HidePopup('" + name + "');return false;}\" />";
	html += "<input type=\"button\" name=\"Cancel\" value=\"" + ewLanguage.Phrase("PopupCancel") + "\" onclick=\"ewrpt_HidePopup('" + name + "');return false;\" />";
	html += "&nbsp;</td></tr>";
	html += "</table>";
	html += "<div id='" + name + "_ResizeGrip' class='ewPopupSizeGrip'><img src=\"" + EWRPT_IMAGES_FOLDER + "/resize.gif\" width=\"9\" height=\"9\" border=\"0\"></div>";
	html += "</form>";
