...
// Create page object
$paymentlog_edit = new cpaymentlog_edit();
$Page =& $paymentlog_edit;

// Page init
$paymentlog_edit->Page_Init();

// Page main
$paymentlog_edit->Page_Main();
?>
...
// Create page object
var paymentlog_edit = new ew_Page("paymentlog_edit");

// page properties
paymentlog_edit.PageID = "edit"; // page ID
paymentlog_edit.FormID = "fpaymentlogedit"; // form ID
var EW_PAGE_ID = paymentlog_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
paymentlog_edit.ValidateForm = function(fobj) {
...
paymentlog_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
paymentlog_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
paymentlog_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
paymentlog_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<div class="phpmaker ewTitle"><img src="images/ico_paid.png" width="40" height="40" align="absmiddle" /><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $paymentlog->TableCaption() ?></div>
<div class="clear"></div>
<?php $paymentlog_edit->ShowPageHeader(); ?>
<?php
$paymentlog_edit->ShowMessage();
?>
<form name="fpaymentlogedit" id="fpaymentlogedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return paymentlog_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="paymentlog">
...
<?php if ($paymentlog->pay_date->Visible) { // pay_date ?>
	<tr id="r_pay_date"<?php echo $paymentlog->RowAttributes() ?>>
		<td width="170" class="ewTableHeader"><?php echo $paymentlog->pay_date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td<?php echo $paymentlog->pay_date->CellAttributes() ?>><span id="el_pay_date">
<input type="text" name="x_pay_date" id="x_pay_date" value="<?php echo $paymentlog->pay_date->EditValue ?>"<?php echo $paymentlog->pay_date->EditAttributes() ?>>
...
</span><?php echo $paymentlog->t_code->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($paymentlog->village_id->Visible) { // village_id ?>
	<tr id="r_village_id"<?php echo $paymentlog->RowAttributes() ?>>
		<td class="ewTableHeader"><?php echo $paymentlog->village_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td<?php echo $paymentlog->village_id->CellAttributes() ?>><span id="el_village_id">
...
<select id="x_pay_type" name="x_pay_type"<?php echo $paymentlog->pay_type->EditAttributes() ?>  style="font-size:18px; border: solid 1px #ccc;" onchange="getunitrate(options[selectedIndex].value);">
<?php
...
<?php if ($paymentlog->pay_detail->Visible) { // pay_detail ?>
	<tr id="r_pay_detail"<?php echo $paymentlog->RowAttributes() ?>>
		<td class="ewTableHeader"><?php echo $paymentlog->pay_detail->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td<?php echo $paymentlog->pay_detail->CellAttributes() ?>><span id="el_pay_detail">
<input type="text" name="x_pay_detail" id="x_pay_detail" size="10" value="<?php echo $paymentlog->pay_detail->EditValue ?>"<?php echo $paymentlog->pay_detail->EditAttributes() ?> style="font-size:22px; border: solid 1px #999;" onchange="recal();">
...
<input type="text" name="x_count_member" id="x_count_member" size="20" value="<?php echo $paymentlog->count_member->EditValue ?>"<?php echo $paymentlog->count_member->EditAttributes() ?> style="font-size:22px; border: solid 1px #999;" onchange="recal();">
...
<input type="text" name="x_pay_rate" id="x_pay_rate" size="20" value="<?php echo $paymentlog->pay_rate->EditValue ?>"<?php echo $paymentlog->pay_rate->EditAttributes() ?> style="font-size:22px; border: solid 1px #999;" onchange="recal();">
...
<input type="text" name="x_sub_total" id="x_sub_total" size="20" value="<?php echo $paymentlog->sub_total->EditValue ?>"<?php echo $paymentlog->sub_total->EditAttributes() ?> style="font-size:22px; border: solid 1px #999;" >
...
<input type="text" name="x_assc_rate" id="x_assc_rate" size="3" value="<?php echo $paymentlog->assc_rate->EditValue ?>"<?php echo $paymentlog->assc_rate->EditAttributes() ?> style="font-size:22px; border: solid 1px #999;" onchange="recal();">
...
<input type="text" name="x_assc_total" id="x_assc_total" size="20" value="<?php echo $paymentlog->assc_total->EditValue ?>"<?php echo $paymentlog->assc_total->EditAttributes() ?> style="font-size:22px; border: solid 1px #999;">
...
<input type="text" name="x_grand_total" id="x_grand_total" size="20" value="<?php echo $paymentlog->grand_total->EditValue ?>"<?php echo $paymentlog->grand_total->EditAttributes() ?> style="font-size:22px; border: solid 1px #999;">
...
<textarea name="x_pay_note" id="x_pay_note" cols="35" rows="4"<?php echo $paymentlog->pay_note->EditAttributes() ?>><?php echo $paymentlog->pay_note->EditValue ?></textarea>
...
<input type="text" name="x_pml_slipt_num" id="x_pml_slipt_num" size="30" value="<?php echo $paymentlog->pml_slipt_num->EditValue ?>"<?php echo $paymentlog->pml_slipt_num->EditAttributes() ?>>
...
<input type="hidden" name="x_pml_id" id="x_pml_id" value="<?php echo ew_HtmlEncode($paymentlog->pml_id->CurrentValue) ?>">
<p>
...
<?php
$paymentlog_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$paymentlog_edit->Page_Terminate();
?>
<?php
...
class cpaymentlog_edit {
	...
	// Page object name
	var $PageObjName = 'paymentlog_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$paymentlog->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$paymentlog->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$paymentlog->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$paymentlog->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($paymentlog->pml_id->CurrentValue == "")
			$this->Page_Terminate("paymentloglist.php"); // Invalid key, return to list
		switch ($paymentlog->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$paymentlog->SendEmail = TRUE; // Send email on update success
				if ($this->EditRow()) { // Update record based on key
					$this->setSuccessMessage($Language->Phrase("UpdateSuccess")); // Update success
					$sReturnUrl = $paymentlog->ViewUrl();
					if (ew_GetPageName($sReturnUrl) == "paymentlogview.php")
						$sReturnUrl = $paymentlog->ViewUrl(); // View paging, return to View page directly
		...
		// Render the record
		$paymentlog->RowType = EW_ROWTYPE_EDIT; // Render as Edit
	...
	function LoadFormValues() {
		...
		if (!$paymentlog->pml_id->FldIsDetailKey)
			$paymentlog->pml_id->setFormValue($objForm->GetValue("x_pml_id"));
	}
	...
	function RestoreFormValues() {
		global $objForm, $paymentlog;
		$this->LoadRow();
		$paymentlog->pml_id->CurrentValue = $paymentlog->pml_id->FormValue;
	...
	function RenderRow() {
		...
		} elseif ($paymentlog->RowType == EW_ROWTYPE_EDIT) { // Edit row
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// pay_date
			$paymentlog->pay_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($paymentlog->pay_date->CurrentValue, 7), ew_CurrentDate(), $paymentlog->pay_date->ReadOnly);

			// t_code
			$paymentlog->t_code->SetDbValueDef($rsnew, $paymentlog->t_code->CurrentValue, "", $paymentlog->t_code->ReadOnly);

			// village_id
			$paymentlog->village_id->SetDbValueDef($rsnew, $paymentlog->village_id->CurrentValue, 0, $paymentlog->village_id->ReadOnly);

			// pay_type
			$paymentlog->pay_type->SetDbValueDef($rsnew, $paymentlog->pay_type->CurrentValue, 0, $paymentlog->pay_type->ReadOnly);

			// pay_detail
			$paymentlog->pay_detail->SetDbValueDef($rsnew, $paymentlog->pay_detail->CurrentValue, "", $paymentlog->pay_detail->ReadOnly);

			// count_member
			$paymentlog->count_member->SetDbValueDef($rsnew, $paymentlog->count_member->CurrentValue, 0, $paymentlog->count_member->ReadOnly);

			// pay_rate
			$paymentlog->pay_rate->SetDbValueDef($rsnew, $paymentlog->pay_rate->CurrentValue, 0, $paymentlog->pay_rate->ReadOnly);

			// sub_total
			$paymentlog->sub_total->SetDbValueDef($rsnew, $paymentlog->sub_total->CurrentValue, 0, $paymentlog->sub_total->ReadOnly);

			// assc_rate
			$paymentlog->assc_rate->SetDbValueDef($rsnew, $paymentlog->assc_rate->CurrentValue, 0, $paymentlog->assc_rate->ReadOnly);

			// assc_total
			$paymentlog->assc_total->SetDbValueDef($rsnew, $paymentlog->assc_total->CurrentValue, 0, $paymentlog->assc_total->ReadOnly);

			// grand_total
			$paymentlog->grand_total->SetDbValueDef($rsnew, $paymentlog->grand_total->CurrentValue, 0, $paymentlog->grand_total->ReadOnly);

			// pay_note
			$paymentlog->pay_note->SetDbValueDef($rsnew, $paymentlog->pay_note->CurrentValue, NULL, $paymentlog->pay_note->ReadOnly);

			// pml_slipt_num
			$paymentlog->pml_slipt_num->SetDbValueDef($rsnew, $paymentlog->pml_slipt_num->CurrentValue, 0, $paymentlog->pml_slipt_num->ReadOnly);

			// Call Row Updating event
			$bUpdateRow = $paymentlog->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				if (count($rsnew) > 0)
					$EditRow = $conn->Execute($paymentlog->UpdateSQL($rsnew));
				else
		...
		if ($EditRow)
			$paymentlog->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
